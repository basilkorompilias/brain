...
*Brand Brain MCP server, demoed in Cursor. Target: ~5 minutes.*
...
- `cd brand-brain && pip install -e .`
- Open the `brand-brain` folder in Cursor.
- Cursor, Settings, MCP, confirm brand-brain shows 4 tools (green).
- Run `python tests/test_validator.py` once on camera. 8/8 passed.

One line to say: "The intelligence isn't in a giant prompt. It's in three tools any model can call. I'm using Cursor. The same server runs in Claude Desktop unchanged. That portability is the whole point."
...
Run directly, no model needed:
```bash
python -c "from brand_brain import server; print(server.validate_copy('lyra','Book now! The most luxurious paradise. Amazing!'))"
```